...
export class FilteredCardsPipe implements PipeTransform {
  ...
  transform(allCards: response.items[]): SearchResults['items']{

    if(instanceDataService.isSortByDateClicked()) {

      if(instanceDataService.getSortValue()) {
        return allCards.filter(card => card.snippet.tags.join().indexOf(instanceDataService.getInputValue()) != -1)
          .filter(card => card.snippet.title.toLowerCase().indexOf(instanceDataService.getSortValue()) != -1)
